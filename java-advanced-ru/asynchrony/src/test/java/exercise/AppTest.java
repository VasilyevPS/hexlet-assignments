...
class AppTest {
    ...
    void testGetSize() throws Exception {
        CompletableFuture<Long> result1 = App.getDirectorySize(
                "src/test/resources"
        );
        assertThat(result1.get()).isEqualTo(13);

        CompletableFuture<Long> result2 = App.getDirectorySize(
                "src/test/resources/dir"
        );
        assertThat(result2.get()).isEqualTo(26);

        CompletableFuture<Long> result3 = App.getDirectorySize(
                "src/test/resources/dir/nested"
        );
        assertThat(result3.get()).isEqualTo(51);
    }
    // END
}
