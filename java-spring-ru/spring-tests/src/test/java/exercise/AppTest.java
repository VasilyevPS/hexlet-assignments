...
public class AppTest {
    ...
    void testGetPeople() throws Exception {
        ...
        assertThat(response.getStatus()).isEqualTo(200);
        assertThat(response.getContentType()).isEqualTo(MediaType.APPLICATION_JSON.toString());
        assertThat(response.getContentAsString()).contains("Simpson");
    }

    @Test
    void testUpdatePerson() throws Exception {
        var existingUserEmail = "jessika@yahoo.com";
        var existingUserId = TestUtils.getUserIdByEmail(mockMvc, existingUserEmail);
        MockHttpServletResponse responsePatch = mockMvc
                .perform(patch("/people/{id}", existingUserId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"firstName\": \"Homer\", \"lastName\": \"Simpson\", \"email\": \"homer@ya.ru\"}")
                )
                .andReturn()
                .getResponse();
